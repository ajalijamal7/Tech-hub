$(function () {
    ...
    function renderCart() {
        ...
        $("#full-name").val(currentUser.name.charAt(0).toUpperCase() + currentUser.name.slice(1));
        ...
        $.each(cart, function (index, product) {
            ...
            `);
            let $img = $("<img>").attr({ src: product.image, alt: product.name });
            let $controlsDiv = $("<div>").addClass("quantity-control");
            let $qtySpan = $("<span>").text(product.quantity);

            if (product.quantity === 1) {
                let $plusBtn = $("<button>").addClass("quantity-btn").attr("title", "Increase quantity").text("+")
                    .on("click", function () { changeQuantity(index, +1); });
                let $deleteBtn = $("<button>").addClass("delete-btn").attr("title", "Delete item")
                    .html('<span class="material-symbols-outlined">delete</span>')
                    .on("click", function () { deleteItem(index); });
                $controlsDiv.append($plusBtn, $qtySpan, $deleteBtn);
            } else {
                let $plusBtn = $("<button>").addClass("quantity-btn").attr("title", "Increase quantity").text("+")
                    .on("click", function () { changeQuantity(index, +1); });
                let $minusBtn = $("<button>").addClass("quantity-btn").attr("title", "Decrease quantity").text("-")
                    .on("click", function () { changeQuantity(index, -1); });
    ...
    $("#checkout-form").on("submit", function (e) {
        ...
        localStorage.setItem("allCarts", JSON.stringify(allCarts));
        renderCart();
        $("#phone-number, #delivery-address").val("");
        alert("Order Placed!");
    });
